...
class Exaone4_5_MTPLayer(GradientCheckpointingLayer):
    def __init__(self, config: Exaone4_5_Config):
        super().__init__()
        self.config = config
        self.pre_fc_norm_embedding = Exaone4_5_RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.pre_fc_norm_hidden = Exaone4_5_RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.fc = nn.Linear(config.hidden_size * 2, config.hidden_size, bias=False)
        self.layers = nn.ModuleList(
            [Exaone4_5_DecoderLayer(config, config.num_hidden_layers + i) for i in range(config._num_mtp_layers)]
        )
...
@dataclass
class Exaone4_5_CausalLMOutputWithPast(CausalLMOutputWithPast):
    """
